...
class HPXML2toHEScoreTranslator(HPXMLtoHEScoreTranslatorBase):
    ...
    def get_attic_knee_wall_rvalue_and_area(self, attic, b):
        ...
        if len(knee_walls) == 0:
            return 0, 0
        if len(knee_walls) == 1:
            return convert_to_type(float, self.xpath(knee_walls[0], 'sum(h:Insulation/h:Layer/h:NominalRValue)')), convert_to_type(float, self.xpath(knee_walls[0], 'h:Area/text()'))
