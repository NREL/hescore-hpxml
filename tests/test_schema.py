...
def test_invalid_building_about(hpxml_filebase):
    ...
    if hpxml_filebase == 'townhouse_walls':
        ...
        errors = get_error_messages(js3, js_schema)
        assert ("{'assessment_date': '2021', 'shape': 'rectangle', 'town_house_walls': 'back_front_left', "
                "'year_built': 1961, 'number_bedrooms': 4, 'num_floor_above_grade': 2, 'floor_to_ceiling_height': 7, "
                "'conditioned_floor_area': 2400, 'orientation': 'north', 'blower_door_test': True, "
                "'envelope_leakage': 1204} should not be valid under {'required': ['town_house_walls']}") in errors
        js3['building']['about']['air_sealing_present'] = True
        errors = get_error_messages(js3, js_schema)
        assert ("{'assessment_date': '2021', 'shape': 'rectangle', 'town_house_walls': 'back_front_left', "
                "'year_built': 1961, 'number_bedrooms': 4, 'num_floor_above_grade': 2, 'floor_to_ceiling_height': 7, "
                "'conditioned_floor_area': 2400, 'orientation': 'north', 'blower_door_test': True, "
                "'envelope_leakage': 1204, 'air_sealing_present': True} should not be valid under "
                "{'required': ['air_sealing_present']}") in errors
    elif hpxml_filebase == 'house1':
        js3['building']['about']['envelope_leakage'] = 1204
        errors = get_error_messages(js3, js_schema)
        assert ("{'assessment_date': '2021', 'shape': 'rectangle', 'year_built': 1953, 'number_bedrooms': 3, "
                ...
                "'envelope_leakage': 1204} should not be valid under {'required': ['envelope_leakage']}") in errors
...
def test_invalid_roof(hpxml_filebase):
    ...
    errors = get_error_messages(js2, js_schema)
    if hpxml_filebase == 'townhouse_walls':
        assert ("{'roof_name': 'roof1', 'roof_area': 1200.0, 'roof_assembly_code': 'rfwf00co', 'roof_color': 'medium', "
                ...
                "'solar_screen': False}, 'roof_absorptance': 0.6} should not be valid under "
                "{'required': ['ceiling_assembly_code']}") in errors
        assert ("{'roof_name': 'roof1', 'roof_area': 1200.0, 'roof_assembly_code': 'rfwf00co', 'roof_color': 'medium', "
                "'roof_type': 'cath_ceiling', 'ceiling_assembly_code': 'ecwf49', "
                "'zone_skylight': {'skylight_area': 11.0, 'skylight_method': 'code', 'skylight_code': 'dcab', "
                "'solar_screen': False}, 'roof_absorptance': 0.6} should not be valid under "
                "{'required': ['roof_absorptance']}") in errors
    elif hpxml_filebase == 'house1':
        assert ("{'roof_name': 'roof1', 'roof_area': 810, "
                ...
                "'roof_absorptance': 0.6} should not be valid under {'required': ['ceiling_assembly_code']}") in errors
        assert ("{'roof_name': 'roof1', 'roof_area': 810, "
                "'roof_assembly_code': 'rfrb00co', 'roof_color': 'dark', 'roof_type': 'cath_ceiling', "
                "'ceiling_assembly_code': 'ecwf11', 'zone_skylight': {'skylight_area': 0}, "
                "'roof_absorptance': 0.6} should not be valid under {'required': ['roof_absorptance']}") in errors
...
def test_invalid_skylight():
    ...
    assert ("{'skylight_area': 11.0, 'skylight_method': 'custom', "
            "'skylight_code': 'dcab', 'solar_screen': False} should not be valid under "
            "{'required': ['skylight_code']}") in errors
    ...
    errors = get_error_messages(js3, js_schema)
    assert ("{'skylight_area': 11.0, 'skylight_method': 'code', "
            "'skylight_code': 'dcab', 'solar_screen': False, 'skylight_u_value': 0.5} should not be valid under "
            "{'required': ['skylight_u_value']}") in errors
...
def test_invalid_floor(hpxml_filebase):
    ...
    errors = get_error_messages(js2, js_schema)
    if hpxml_filebase == 'townhouse_walls':
        assert ("{'floor_name': 'floor1', 'floor_area': 800, 'foundation_type': 'slab_on_grade', "
                "'foundation_insulation_level': 0, 'floor_assembly_code': 'efwf00ca'} should not be valid under "
                "{'required': ['floor_assembly_code']}") in errors
    elif hpxml_filebase == 'house1':
        assert ("{'floor_name': 'floor1', 'floor_area': 810.0, 'foundation_type': 'slab_on_grade', "
                "'foundation_insulation_level': 0, 'floor_assembly_code': 'efwf00ca'} should not be valid under "
                "{'required': ['floor_assembly_code']}") in errors
...
def test_invalid_heating(hpxml_filebase):
    ...
    assert ("{'fuel_primary': 'natural_gas', 'type': 'central_furnace', 'efficiency_method': 'shipment_weighted', "
            "'efficiency': 0.5} should not be valid under {'required': ['efficiency']}") in errors
    ...
    if hpxml_filebase == 'townhouse_walls':
        assert ("{'fuel_primary': 'natural_gas', 'type': 'central_furnace', 'efficiency_method': 'user', "
                "'efficiency': 0.95, 'year': 2021} should not be valid under {'required': ['year']}") in errors
    elif hpxml_filebase == 'house1':
        assert ("{'fuel_primary': 'natural_gas', 'type': 'central_furnace', 'efficiency_method': 'user', "
                "'efficiency': 0.92, 'year': 2021} should not be valid under {'required': ['year']}") in errors
...
def test_invalid_cooling(hpxml_filebase):
    ...
    errors = get_error_messages(js1, js_schema)
    assert "'hvac_name' is a required property" in errors
    assert ("{'type': 'split_dx', 'efficiency_method': 'user', "
            "'efficiency': 13.0, 'year': 2021} should not be valid under {'required': ['year']}") in errors
    ...
    assert ("{'type': 'split_dx', 'efficiency_method': 'shipment_weighted', 'efficiency': 7.9} should not be valid "
            "under {'required': ['efficiency']}") in errors
    # heat pump
...
def test_invalid_hvac_distribution(hpxml_filebase):
    ...
    errors = get_error_messages(js1, js_schema)
    if hpxml_filebase == 'townhouse_walls':
        assert ("{'hvac_name': 'hvac1', 'hvac_fraction': 1.0, 'heating': {'fuel_primary': 'natural_gas', "
                "'type': 'wall_furnace', 'efficiency_method': 'user', 'efficiency': 0.95}, "
                "'cooling': {'type': 'packaged_dx', 'efficiency_method': 'user', 'efficiency': 13.0}, "
                "'hvac_distribution': {'leakage_method': 'qualitative', 'sealed': False, "
                "'duct': [{'name': 'duct1', 'fraction': 1.0}]}} should not be valid under "
                "{'required': ['hvac_distribution']}") in errors
    elif hpxml_filebase == 'house1':
        assert ("{'hvac_name': 'hvac1', 'hvac_fraction': 1.0, "
                ...
                "'insulated': False}]}} should not be valid under {'required': ['hvac_distribution']}") in errors
    ...
    if hpxml_filebase == 'townhouse_walls':
        assert ("{'sealed': False, 'duct': [{'name': 'duct1', "
                "'location': 'cond_space', 'fraction': 1.0, 'insulated': False}], "
                "'leakage_method': 'quantitative'} should not be valid under {'required': ['sealed']}") in errors
    elif hpxml_filebase == 'house1':
        assert ("{'sealed': False, 'duct': [{'name': 'duct1', "
...
def test_invalid_solar_electric(hpxml_filebase):
    ...
    assert ("{'capacity_known': False, 'system_capacity': 50} should not be valid under "
            "{'required': ['system_capacity']}") in errors
    ...
    assert "'year' is a required property" in errors
    assert "'array_azimuth' is a required property" in errors
    assert ("{'capacity_known': True, 'num_panels': 5} should not be valid under "
            "{'required': ['num_panels']}") in errors
